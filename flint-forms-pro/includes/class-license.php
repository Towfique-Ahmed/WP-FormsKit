...
class License {
	...
	public function handle_form() {
		...
		if ( ! current_user_can( 'manage_options' ) || ! isset( $_POST['formskit_license_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['formskit_license_nonce'] ) ), 'formskit_license' ) ) {
			wp_die( esc_html__( 'Security check failed.', 'flint-forms-pro' ) );
		}
	...
	public function render_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Flint Forms Pro License', 'flint-forms-pro' ); ?></h1>

			<form method="post">
				<?php wp_nonce_field( 'formskit_license', 'formskit_license_nonce' ); ?>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'License Key', 'flint-forms-pro' ); ?></th>
						<td>
							<input type="text" class="regular-text" name="formskit_license_key" value="<?php echo esc_attr( $license['key'] ); ?>" <?php disabled( $active ); ?> />
							<p class="description">
								<?php
								if ( $active ) {
									echo '<span style="color:#15803d;font-weight:600;">' . esc_html__( 'Active', 'flint-forms-pro' ) . '</span>';
								} elseif ( 'invalid' === $license['status'] ) {
									echo '<span style="color:#b91c1c;font-weight:600;">' . esc_html__( 'Invalid key', 'flint-forms-pro' ) . '</span>';
								} else {
									esc_html_e( 'Enter your license key to receive automatic updates.', 'flint-forms-pro' );
								}
								?>
							</p>
						</td>
				...
				<?php if ( $active ) : ?>
					<button type="submit" name="formskit_license_action" value="deactivate" class="button"><?php esc_html_e( 'Deactivate', 'flint-forms-pro' ); ?></button>
				<?php else : ?>
					<button type="submit" name="formskit_license_action" value="activate" class="button button-primary"><?php esc_html_e( 'Activate License', 'flint-forms-pro' ); ?></button>
				<?php endif; ?>
			</form>
		</div>
		<?php
