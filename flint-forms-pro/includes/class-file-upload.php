...
class File_Upload {
	...
	public function register( $fields ) {
		$fields->register(
			'file',
			array(
				'label'  => __( 'File Upload', 'flint-forms-pro' ),
	...
	public function validate( $error, $field, $value ) {
		...
		if ( ! empty( $field['required'] ) && ! $has_upload ) {
			return __( 'Please choose a file to upload.', 'flint-forms-pro' );
		}
		...
		if ( $has_upload ) {
			...
			if ( UPLOAD_ERR_OK !== $err ) {
				return __( 'The file could not be uploaded. Please try again.', 'flint-forms-pro' );
			}
		}
