...
class Recaptcha {
	...
	public function settings_fields( $settings ) {
		?>
		<tr>
			<th scope="row"><?php esc_html_e( 'reCAPTCHA v3 Site Key', 'flint-forms-pro' ); ?></th>
		...
		<tr>
			<th scope="row"><?php esc_html_e( 'reCAPTCHA v3 Secret Key', 'flint-forms-pro' ); ?></th>
	...
	public function verify( $gate, $config, $request ) {
		...
		if ( '' === $token ) {
			return new \WP_Error( 'formskit_recaptcha', __( 'reCAPTCHA verification failed. Please try again.', 'flint-forms-pro' ) );
		}
		...
		if ( empty( $body['success'] ) || ( isset( $body['score'] ) && (float) $body['score'] < $threshold ) ) {
			return new \WP_Error( 'formskit_recaptcha', __( 'Your submission looked automated and was blocked.', 'flint-forms-pro' ) );
		}
