( function ( wp ) {
	...
	var options = [ { label: __( '— Select a form —', 'wp-formskit' ), value: 0 } ].concat(
	...
	registerBlockType( 'formskit/form', {
		edit: function ( props ) {
			...
			var inspector = el(
				...
				el(
					PanelBody,
					{ title: __( 'Form Settings', 'wp-formskit' ) },
					el( SelectControl, {
						label: __( 'Select form', 'wp-formskit' ),
						value: formId,
						options: options,
			...
			if ( ! formId ) {
				body = el(
					...
					{
						icon: 'feedback',
						label: __( 'WP FormsKit', 'wp-formskit' ),
						instructions: __( 'Choose a form to display.', 'wp-formskit' ),
					},
					el( SelectControl, {
						value: formId,
						options: options,
