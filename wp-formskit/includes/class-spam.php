...
class Spam {
	...
	public static function check( array $config, array $request ) {
		...
		if ( ! empty( $spam['honeypot'] ) && ! empty( $request[ self::HONEYPOT_FIELD ] ) ) {
			return new \WP_Error( 'formskit_spam', __( 'Your submission was flagged as spam.', 'wp-formskit' ) );
		}
		...
		if ( $min_time > 0 && ! empty( $request['formskit_ts'] ) ) {
			...
			if ( $elapsed < $min_time ) {
				return new \WP_Error( 'formskit_spam', __( 'Your submission was sent too quickly. Please try again.', 'wp-formskit' ) );
			}
		}
	...
	public static function render_honeypot() {
		printf(
			...
			esc_html__( 'Leave this field empty', 'wp-formskit' ),
