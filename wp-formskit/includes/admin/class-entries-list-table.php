...
class Entries_List_Table extends \WP_List_Table {
	...
	public function get_columns() {
		return array(
			...
			'summary'    => __( 'Entry', 'wp-formskit' ),
			'form'       => __( 'Form', 'wp-formskit' ),
			'status'     => __( 'Status', 'wp-formskit' ),
			'created_at' => __( 'Date', 'wp-formskit' ),
		);
	}
	...
	public function column_summary( $item ) {
		...
		$actions = array(
			'view'   => sprintf( '<a href="%s">%s</a>', esc_url( $view_url ), esc_html__( 'View', 'wp-formskit' ) ),
			'delete' => sprintf( '<a href="%s" class="submitdelete" onclick="return confirm(\'%s\');">%s</a>', esc_url( $delete_url ), esc_js( __( 'Delete this entry?', 'wp-formskit' ) ), esc_html__( 'Delete', 'wp-formskit' ) ),
		);
	...
	public function column_status( $item ) {
		$map = array(
			'unread' => __( 'Unread', 'wp-formskit' ),
			'read'   => __( 'Read', 'wp-formskit' ),
			'spam'   => __( 'Spam', 'wp-formskit' ),
		);
	...
	public function no_items() {
		esc_html_e( 'No entries found yet.', 'wp-formskit' );
	}
	...
	public function form_filter_dropdown() {
		...
		echo '<option value="0">' . esc_html__( 'All forms', 'wp-formskit' ) . '</option>';
		foreach ( $forms as $form ) {
			printf( '<option value="%d" %s>%s</option>', (int) $form->ID, selected( $current, $form->ID, false ), esc_html( $form->post_title ) );
		}
		echo '</select> <button class="button">' . esc_html__( 'Filter', 'wp-formskit' ) . '</button></div>';
	}
}
