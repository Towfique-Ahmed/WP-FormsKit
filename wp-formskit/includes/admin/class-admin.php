...
class Admin {
	...
	public function assets( $hook ) {
		...
		wp_localize_script(
			...
			array(
				'fieldTypes' => $types,
				'i18n'       => array(
					'newField'    => __( 'New Field', 'wp-formskit' ),
					'option'      => __( 'Option', 'wp-formskit' ),
					'confirmDel'  => __( 'Remove this field?', 'wp-formskit' ),
					'untitled'    => __( 'Untitled Field', 'wp-formskit' ),
					'proField'    => __( 'This is a Pro field. Upgrade to use it.', 'wp-formskit' ),
				),
	...
	public function render_entries_page() {
		...
		<div class="wrap formskit-entries-wrap">
			<h1 class="wp-heading-inline"><?php esc_html_e( 'Form Entries', 'wp-formskit' ); ?></h1>
			...
			<form method="get">
				...
				$table->search_box( __( 'Search Entries', 'wp-formskit' ), 'formskit-entries' );
				$table->display();
				?>
			</form>
		</div>
	...
	protected function render_single_entry( $entry_id ) {
		...
		if ( ! $entry ) {
			echo '<div class="wrap"><p>' . esc_html__( 'Entry not found.', 'wp-formskit' ) . '</p></div>';
			return;
		}
		...
		<div class="wrap formskit-entry-detail">
			<h1 class="wp-heading-inline">
				<?php printf( esc_html__( 'Entry #%d', 'wp-formskit' ), (int) $entry['id'] ); ?>
			</h1>
			<a href="<?php echo esc_url( $back ); ?>" class="page-title-action"><?php esc_html_e( 'Back to entries', 'wp-formskit' ); ?></a>
			<hr class="wp-header-end" />
			...
			<div class="formskit-entry-columns">
				...
				<div class="formskit-entry-side">
					<div class="postbox"><div class="inside">
						<p><strong><?php esc_html_e( 'Form:', 'wp-formskit' ); ?></strong> <?php echo esc_html( get_the_title( $entry['form_id'] ) ); ?></p>
						<p><strong><?php esc_html_e( 'Submitted:', 'wp-formskit' ); ?></strong> <?php echo esc_html( $entry['created_at'] ); ?></p>
						<p><strong><?php esc_html_e( 'IP:', 'wp-formskit' ); ?></strong> <?php echo esc_html( $entry['ip_address'] ); ?></p>
						<?php if ( ! empty( $entry['meta']['referer'] ) ) : ?>
							<p><strong><?php esc_html_e( 'Page:', 'wp-formskit' ); ?></strong><br /><a href="<?php echo esc_url( $entry['meta']['referer'] ); ?>"><?php echo esc_html( $entry['meta']['referer'] ); ?></a></p>
						<?php endif; ?>
					</div></div>
				</div>
			</div>
	...
	protected function export_button() {
		...
		printf( '<a href="%s" class="page-title-action">%s</a>', esc_url( $url ), esc_html__( 'Export CSV', 'wp-formskit' ) );
	}
	...
	public function maybe_export_csv() {
		...
		if ( ! current_user_can( 'edit_posts' ) || ! isset( $_GET['formskit_export_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['formskit_export_nonce'] ) ), 'formskit_export' ) ) {
			wp_die( esc_html__( 'Security check failed.', 'wp-formskit' ) );
		}
