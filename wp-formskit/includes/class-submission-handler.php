...
class Submission_Handler {
	...
	public function process( array $request ) {
		...
		if ( ! $form_id || Post_Type::POST_TYPE !== get_post_type( $form_id ) ) {
			return new \WP_Error( 'formskit_invalid_form', __( 'Invalid form.', 'wp-formskit' ) );
		}
		...
		if ( ! wp_verify_nonce( $nonce, 'formskit_submit_' . $form_id ) ) {
			return new \WP_Error( 'formskit_bad_nonce', __( 'Security check failed. Please reload and try again.', 'wp-formskit' ) );
		}
		...
		if ( $errors ) {
			return new \WP_Error(
				'formskit_validation',
				__( 'Please correct the highlighted fields.', 'wp-formskit' ),
				$errors
			);
		}
