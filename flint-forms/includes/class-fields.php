...
class Fields {
	...
	protected function register_defaults() {
		...
		$this->register( 'text', array( 'label' => __( 'Single Line Text', 'flint-forms' ), 'icon' => 'dashicons-editor-textcolor' ) );
		$this->register( 'textarea', array( 'label' => __( 'Paragraph Text', 'flint-forms' ), 'icon' => 'dashicons-editor-paragraph', 'render' => array( $this, 'render_textarea' ) ) );
		$this->register( 'email', array( 'label' => __( 'Email', 'flint-forms' ), 'icon' => 'dashicons-email', 'sanitize' => 'sanitize_email', 'validate' => array( $this, 'validate_email' ) ) );
		$this->register( 'number', array( 'label' => __( 'Number', 'flint-forms' ), 'icon' => 'dashicons-calculator' ) );
		$this->register( 'tel', array( 'label' => __( 'Phone', 'flint-forms' ), 'icon' => 'dashicons-phone' ) );
		$this->register( 'url', array( 'label' => __( 'Website / URL', 'flint-forms' ), 'icon' => 'dashicons-admin-links', 'sanitize' => 'esc_url_raw' ) );
		$this->register( 'date', array( 'label' => __( 'Date', 'flint-forms' ), 'icon' => 'dashicons-calendar-alt' ) );
		$this->register( 'name', array( 'label' => __( 'Name', 'flint-forms' ), 'icon' => 'dashicons-admin-users' ) );
		$this->register( 'select', array( 'label' => __( 'Dropdown', 'flint-forms' ), 'icon' => 'dashicons-menu', 'has_options' => true, 'render' => array( $this, 'render_select' ) ) );
		$this->register( 'radio', array( 'label' => __( 'Multiple Choice', 'flint-forms' ), 'icon' => 'dashicons-marker', 'has_options' => true, 'render' => array( $this, 'render_choices' ) ) );
		$this->register( 'checkbox', array( 'label' => __( 'Checkboxes', 'flint-forms' ), 'icon' => 'dashicons-yes', 'has_options' => true, 'render' => array( $this, 'render_choices' ), 'sanitize' => array( $this, 'sanitize_array' ) ) );
		$this->register( 'hidden', array( 'label' => __( 'Hidden', 'flint-forms' ), 'icon' => 'dashicons-hidden', 'render' => array( $this, 'render_hidden' ) ) );
		$this->register( 'html', array( 'label' => __( 'HTML Block', 'flint-forms' ), 'icon' => 'dashicons-editor-code', 'group' => 'layout', 'render' => array( $this, 'render_html' ) ) );
	...
	public function validate( array $field, $value ) {
		...
		if ( ! empty( $field['required'] ) && $is_empty ) {
			return __( 'This field is required.', 'flint-forms' );
		}
	...
	public function validate_email( $value ) {
		return is_email( $value ) ? '' : __( 'Please enter a valid email address.', 'flint-forms' );
	}
	...
	public function render_select( $field, $value, $name ) {
		...
		$out .= '<option value="">' . esc_html( isset( $field['placeholder'] ) && $field['placeholder'] ? $field['placeholder'] : __( '— Select —', 'flint-forms' ) ) . '</option>';
