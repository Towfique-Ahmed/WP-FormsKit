...
function get_form_config( $form_id ) {
	...
	$config['settings'] = wp_parse_args(
		$config['settings'],
		array(
			'submit_label'  => __( 'Submit', 'flint-forms' ),
			'ajax'          => true,
			'store_entries' => true,
			'confirmation'  => array(
				'type'         => 'message',
				'message'      => __( 'Thanks! Your submission has been received.', 'flint-forms' ),
				'redirect_url' => '',
			),
			'notification'  => array(
				'enabled'  => true,
				'to'       => '{admin_email}',
				'subject'  => sprintf( /* translators: %s: form title */ __( 'New submission: %s', 'flint-forms' ), $config['title'] ),
				'message'  => '{all_fields}',
				'reply_to' => '',
			),
			'spam'          => array(
