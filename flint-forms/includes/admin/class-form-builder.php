...
class Form_Builder {
	...
	public function render_builder( $post ) {
		...
		<div id="formskit-builder-app" class="formskit-builder">
			<div class="formskit-builder-toolbar">
				<button type="button" class="button" data-formskit-tab="fields"><?php esc_html_e( 'Fields', 'flint-forms' ); ?></button>
				<button type="button" class="button" data-formskit-tab="settings"><?php esc_html_e( 'Settings', 'flint-forms' ); ?></button>
				<button type="button" class="button" data-formskit-tab="notifications"><?php esc_html_e( 'Notifications', 'flint-forms' ); ?></button>
			</div>

			<div class="formskit-builder-body">
				<aside class="formskit-field-palette">
					<h3><?php esc_html_e( 'Add Fields', 'flint-forms' ); ?></h3>
				...
				<main class="formskit-builder-canvas">
					<div class="formskit-panel" data-panel="fields">
						...
						<p class="formskit-empty-hint"><?php esc_html_e( 'Click a field on the left to add it here.', 'flint-forms' ); ?></p>
					</div>
	...
	public function render_embed( $post ) {
		...
		?>
		<p><?php esc_html_e( 'Copy this shortcode into any post, page or widget:', 'flint-forms' ); ?></p>
		<input type="text" class="widefat code" readonly value="<?php echo esc_attr( $shortcode ); ?>" onclick="this.select();" />
		<p class="description"><?php esc_html_e( 'Or add the “Flint Forms Form” block in the editor.', 'flint-forms' ); ?></p>
		<?php
	}
	...
	protected function sanitize_config( array $config ) {
		...
		$clean['settings'] = array(
			'submit_label'  => isset( $settings['submit_label'] ) && '' !== trim( $settings['submit_label'] ) ? sanitize_text_field( $settings['submit_label'] ) : __( 'Submit', 'flint-forms' ),
