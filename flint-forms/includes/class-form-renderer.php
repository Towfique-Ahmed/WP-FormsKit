	...
	public function render( $form_id, array $args = array() ) {
		...
		if ( ! $form_id || Post_Type::POST_TYPE !== get_post_type( $form_id ) ) {
			return current_user_can( 'edit_posts' )
				? '<p class="formskit-notice">' . esc_html__( 'Flint Forms: form not found.', 'flint-forms' ) . '</p>'
				: '';
		}
		...
		if ( empty( $fields ) ) {
			return current_user_can( 'edit_posts' )
				? '<p class="formskit-notice">' . esc_html__( 'Flint Forms: this form has no fields yet.', 'flint-forms' ) . '</p>'
				: '';
		}
	...
	public function enqueue_assets() {
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'    => array(
					'error'      => __( 'Something went wrong. Please try again.', 'flint-forms' ),
					'required'   => __( 'This field is required.', 'flint-forms' ),
					'validating' => __( 'Sending…', 'flint-forms' ),
				),
			)
		);
